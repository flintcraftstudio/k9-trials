...
//
// Run is the reusable entry point, called both by cmd/seed-demo (the CLI:
// mage seeddemo) and by the admin "Reset demo data" endpoint when DEMO_MODE
// is enabled. The caller owns opening and migrating the database.
package seeddemo
...
// Run wipes all application data and reseeds the deterministic demo world.
// The database behind st must already be migrated (goose.Up). It returns a
// human-readable summary of what was created.
func Run(ctx context.Context, st *store.Store) (string, error) {
	conn := st.DB()
